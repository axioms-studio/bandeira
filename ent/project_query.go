...
func (_q *ProjectQuery) QueryFlags() *FlagQuery {
...
func (_q *ProjectQuery) WithFlags(opts ...func(*FlagQuery)) *ProjectQuery {
	query := (&FlagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withFlags; query != nil {
		if err := _q.loadFlags(ctx, query, nodes,
			func(n *Project) { n.Edges.Flags = []*Flag{} },
			func(n *Project, e *Flag) { n.Edges.Flags = append(n.Edges.Flags, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadFlags(ctx context.Context, query *FlagQuery, nodes []*Project, init func(*Project), assign func(*Project, *Flag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(flag.FieldProjectID)
	}
	query.Where(predicate.Flag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.FlagsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ProjectQuery) loadAPITokens(ctx context.Context, query *ApiTokenQuery, nodes []*Project, init func(*Project), assign func(*Project, *ApiToken)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apitoken.FieldProjectID)
	}
