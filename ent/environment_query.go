...
func (_q *EnvironmentQuery) QueryFlagEnvironments() *FlagEnvironmentQuery {
...
func (_q *EnvironmentQuery) WithFlagEnvironments(opts ...func(*FlagEnvironmentQuery)) *EnvironmentQuery {
	query := (&FlagEnvironmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EnvironmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Environment, error) {
	...
	if query := _q.withFlagEnvironments; query != nil {
		if err := _q.loadFlagEnvironments(ctx, query, nodes,
			func(n *Environment) { n.Edges.FlagEnvironments = []*FlagEnvironment{} },
			func(n *Environment, e *FlagEnvironment) {
				n.Edges.FlagEnvironments = append(n.Edges.FlagEnvironments, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *EnvironmentQuery) loadFlagEnvironments(ctx context.Context, query *FlagEnvironmentQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *FlagEnvironment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(flagenvironment.FieldEnvironmentID)
	}
	query.Where(predicate.FlagEnvironment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(environment.FlagEnvironmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
