...
func (_c *ProjectCreate) AddEnvironmentIDs(ids ...int) *ProjectCreate {
...
func (_c *ProjectCreate) AddFlagIDs(ids ...int) *ProjectCreate {
	_c.mutation.AddFlagIDs(ids...)
	return _c
}

// AddFlags adds the "flags" edges to the Flag entity.
func (_c *ProjectCreate) AddFlags(v ...*Flag) *ProjectCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddFlagIDs(ids...)
}

// AddAPITokenIDs adds the "api_tokens" edge to the ApiToken entity by IDs.
func (_c *ProjectCreate) AddAPITokenIDs(ids ...int) *ProjectCreate {
...
func (_c *ProjectCreate) createSpec() (*Project, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.FlagsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
